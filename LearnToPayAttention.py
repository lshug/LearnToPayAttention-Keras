...
class AttentionVGG:
    def __init__(self, att='att1', gmode='concat', compatibilityfunction='pc', height=32, width=32, channels=3, outputclasses=100, weight_decay=0.005):
        ...
        x = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(inp)
        x = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)

        x = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        x = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)

        x = Conv2D(256, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        x = Conv2D(256, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        local1 = Conv2D(256, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)  # batch*x*y*channel
        x = MaxPooling2D((2, 2), strides=(2, 2))(local1)

        x = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        x = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        local2 = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        x = MaxPooling2D((2, 2), strides=(2, 2))(local2)

        x = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        x = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        local3 = Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizer)(x)
        ...
        g = Dense(512, activation='relu', kernel_regularizer=regularizer)(x)  # batch*512
        ...
        g1 = Lambda(lambda lam: K.squeeze(K.batch_dot(K.expand_dims(lam[0], 1), lam[1]), 1))([a1, reshaped1])  # batch*512.
    ...
    def StandardFit(self, datasetname, X, Y):
        startingepoch = 0
        ...
        tboardcb = keras.callbacks.TensorBoard(log_dir='./logs', histogram_freq=0, batch_size=3, write_graph=True, write_grads=False, write_images=False, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None)
        ...
        callbackslist = [scaler, checkpoint, epochprint, tboardcb]
        self.model.fit(X, Y, 3, 3, callbacks=callbackslist, initial_epoch=startingepoch)
...
if __name__ == "__main__":
    a = AttentionVGG(att='att3', gmode='concat', compatibilityfunction='pc', height=32, width=32, channels=3, outputclasses=10)
    X = np.ones((3, 32, 32, 3))
    Y = 3*np.ones((3, 10))
